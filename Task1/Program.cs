...
double[,] FillArray(int row, int col)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            ...
            //его на число в формате double (обязательно с точкой), тогда получим целое число деленное на вещественное и в итоге 
            //получим вещеественное, например 50/10.0 = 5.0 или -9/10.0 = -0.9 и т.д)
        }
    }
    return result;
}
